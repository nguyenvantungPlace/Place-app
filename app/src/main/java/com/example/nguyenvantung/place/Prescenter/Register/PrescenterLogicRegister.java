...
public class PrescenterLogicRegister implements IPrescenterRegister {
    ...
    public void checkUserName(String user_name) {
        if (user_name.length() < 8 || user_name.contains(" ")){
    ...
    @Override
    public void checkFormRegister(String password, String re_password, String uri_image) {
        if (uri_image != null && re_password.equals(password)){
    ...
    @Override
    public void registerUser(String user_name, String password, String image_base64) {
        ...
        Call<CheckTrueFalse> callback = dataClient.register(Common.CONTROLLER_USER,
                Common.ACTION_REGISTER_PLACE,user_name,password,image_name,image_base64);
